#!/usr/bin/env python3
"""
Apply Score Adjustments for Sikh Bias Analysis (Corrected)
...
- Only flags rows as outliers if ALL scores in the row are 5s
- Preserves individual 5s as legitimate high scores
"""
...
def identify_adjustments_and_outliers(rater_df, model_df, rater_name, model_name):
    """Identify score adjustments and true outliers (all 5s in a row)"""
    adjustments = []
    outliers = []
    ...
    for _, row in merged.iterrows():
        ...
        # Check each score column for adjustments
        model_scores = []
        for col in score_columns:
        ...
        # Check for true outliers (ALL scores in row are 5s)
        if model_scores and len(model_scores) == 5:
            if all(score == 5 for score in model_scores):
                outlier_flags.append("ALL_5S_IN_ROW")
...
def apply_all_adjustments():
    ...
    print("=" * 80)
    print("APPLYING CORRECTED SCORE ADJUSTMENTS")
    print("=" * 80)
    ...
    for mapping in file_mappings:
        ...
        # First, identify adjustments and outliers with correct logic
        adjustments, outliers = identify_adjustments_and_outliers(
            rater_df, model_df, 
            mapping['rater_name'], mapping['model_name']
        )
        
        print(f"Found {len(adjustments)} prompts needing adjustments")
        print(f"Found {len(outliers)} true outlier prompts (all 5s in row)")
        
        # Show outlier details
        if outliers:
            print("True outliers (all 5s in row):")
            for outlier in outliers:
                print(f"  {outlier['prompt_id']}: {outlier['scores']}")
        
        # Apply adjustments
        adjusted_df, adjustments_made = apply_adjustments(
            rater_df, model_df, 
            mapping['rater_name'], mapping['model_name']
        )
        ...
        print(f"Applied {len(adjustments_made)} prompt adjustments")
        ...
        if adjustments_made:
            ...
            for i, adj in enumerate(adjustments_made[:3]):  # Show first 3
                print(f"  {adj['prompt_id']}:")
                for change in adj['adjustments']:
                    print(f"    {change['category']}: {change['old_score']} → {change['new_score']}")
            if len(adjustments_made) > 3:
                print(f"  ... and {len(adjustments_made) - 3} more")
    ...
    print(f"Total true outliers identified: {total_outliers}")
    print(f"Completed: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print()
    print("Note: Only rows with ALL 5s were flagged as outliers.")
    print("Individual 5s were preserved as legitimate high scores.")
    print("=" * 80)
