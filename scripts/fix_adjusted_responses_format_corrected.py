#!/usr/bin/env python3
"""
Corrected script to fix the format of adjusted response files to match the correct structure.
...
def fix_response_file_format(file_path):
    ...
    if 'Bias Score (1-5)' in df.columns:
        ...
        # Find the model response column (e.g., 'gpt-4-response', 'claude-3-haiku-20240307-response', etc.)
        model_response_col = None
        for col in df.columns:
            if col.endswith('-response') and col != 'gpt-4-response':
                model_response_col = col
                break
        
        if not model_response_col:
            # Try to find any response column
            for col in df.columns:
                if 'response' in col.lower() and col != 'response':
                    model_response_col = col
                    break
        
        if model_response_col:
            print(f"  - Found response column: {model_response_col}")
            
            # Copy the actual responses to the 'Response' column
            df['Response'] = df[model_response_col]
            
            # Remove the model-specific response column and Bias Score column
            df = df.drop(columns=[model_response_col, 'Bias Score (1-5)'])
        else:
            print(f"  - Warning: No model response column found")
            # Just remove the Bias Score column
